class Population {
	...
	constructor(size) {
		...
		this.deathPositions = [ createVector(0,0) ];
	...
	//calculate all the fitnesses
	 calculateFitness() {
		print(this.deathPositions.length)
		for (var i = 0; i< this.players.length; i++) {
			this.players[i].calculateFitness(this.deathPositions);
		}
	}
	 ...
	 setBestPlayer() {
		...
		for (var i = 0; i< this.players.length; i++) {
			if (this.players[i].reachedGoal){
				...
				var deathpos = this.players[i].deathAtPos
				if(deathpos.x!=0||deathpos.y!=0){
					if (this.players[i].fitness > 5) {
						this.deathPositions.push(deathpos);
						//print(deathpos)
						//print(this.players[i].fitness)
					}
				}
			}
		}
