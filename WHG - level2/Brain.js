	...
	mutate(died, deathStep) {
		...
		for (var i = this.directions.length-1; i >= 0; i--) {
			...
			if (rand < mutationRate) {
				if (random(1)<0.025) {
					//move this direction to the end of the array
					var dir = this.directions[i];
					for(var j = i+1; j<this.directions.length; j++){
						this.directions[j-1] = this.directions[j];
					}
					this.directions[this.directions.length-1] = dir
				}else{					
